...
public class SampleVelocityConfigApp {
    public static void main(String[] args) throws IOException {
        // Load configuration from the YAML file
        Yaml yaml = new Yaml();
        try (InputStream in = SampleVelocityConfigApp.class.getResourceAsStream("/config.yaml")) {
            ...
            String templatePath = "src/main/resources/template.vm";  // Adjust the path as needed

            // Process the Velocity template
            ve.mergeTemplate(templatePath, "UTF-8", context, writer);

            // Define the output CSV file name
            String outputFileName = "velocity-output.csv";
